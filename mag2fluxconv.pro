pro	mag2fluxconv,fname,lameff,weff,flux,fluxerr,teff=teff,wiseab=wiseab,galex=galex,fitgaia=fitgaia
...
for i=0,n_elements(band)-1 do begin
  case strn(band(i)) of
     ...
     'galNUV': begin
             if keyword_set(galex) then mag(i)=mag(i)+20.08-0.07
             lameff(i)=2267. / 1.e4
             weff(i)=732. / 1.e4
             lamflamzp = 3631. * 3e-9 / lameff(i)^2 * lameff(i)
             flux(i)=lamflamzp*10^(-0.4*(mag(i)))  
; http://galexgi.gsfc.nasa.gov/docs/galex/Documents/ERO_data_description_2.htm#_Toc58822546
     ...
     'galFUV': begin
             if keyword_set(galex) then mag(i)=mag(i)+18.82-0.07
     ...
     'WISE4': begin
             lameff(i)=22.8  ; revised as per arxiv:1411.5444
;             lameff(i)=22.0883
             weff(i)=4.1013
             lamflamzp = 3631. * 3e-9 / lameff(i)^2 * lameff(i)
             if keyword_set(wiseab) then begin
